// Adapted from: https://gist.github.com/SagiMedina/f00a57de4e211456225d3114fd10b0d0
...
export default class ImageTools {
  constructor () {
    ...
    resize = (file, maxDimensions) => new Promise((resolve) => {
      ...
      image.onload = () => {
        ...
        EXIF.getData(image, () => {
          ...
          if (hasToBlobSupport) { imageCanvas.toBlob(blob => resolve(blob), file.type) } else { resolve(this.toBlob(imageCanvas, file.type)) }
        })
      }
    ...
    resizeIgnoreAspect = (file, dimensions) => new Promise((resolve, reject) => {
    ...
    drawImageToCanvas = (img, orientation = 1, x = 0, y = 0, width = img.width, height = img.height, method = 'contain') => {
      ...
      if (method === 'crop') { ctx.drawImage(img, (img.width / 2) - (width / 2), (img.height / 2) - (height / 2), width, height, 0, 0, width, height) } else { ctx.drawImage(img, x, y, width, height) }
      ctx.restore()
